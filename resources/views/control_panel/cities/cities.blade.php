...
@section('content')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h4 class="text-themecolor">{{ trans('cities/cities.cities_title') }}</h4>
        </div>
        <div class="col-md-7 align-self-center text-right">
            <div class="d-flex justify-content-end align-items-center">
                @if(!$trashTrigger)
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active"><a href="{{ route('cities.index') }}">{{ trans('cities/cities.cities_title') }}</a></li>
                </ol>
                    <a href="{{ route('cities.create') }}" class="btn btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i>{{ trans('cities/cities.new_city') }}</a>
                    <a href="{{ route('cities.trashed') }}" class="btn btn-danger d-none d-lg-block m-l-15"><i class="fa fa-trash"></i>{{ trans('cities/cities.Deleted_cities') }}</a>
                @else
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item active"><a href="{{ route('cities.trashed') }}">{{ trans('cities/cities.trashed_cities') }}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('cities.index') }}">{{ trans('cities/cities.cities_title') }}</a></li>
                    </ol>
                @endif
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">{{ trans('cities/cities.cities_title') }}</h4>
                        ...
                        <table id="myTable" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    @if($trashTrigger)
                                        <th>
                                            {{ trans('cities/cities.deleted_since') }}
                                        </th>
                                    @endif
                                    {{--<th>add borders</th>--}}
                                    <th>{{ trans('cities/cities.operations') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($cities as $city)
                                    <tr>
                                        ...
                                        @endif
                                        {{--<td>--}}
                                            {{--<a href="{{ route('cities.addAddress',$city->id) }}"><i class="fa fa-map"></i></a>--}}
                                        {{--</td>--}}
                                        <td>
                                            @if(!$trashTrigger)
                                                <a href="{{ route('cities.edit',$city->id) }}" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                                            ...
                                            @else
                                                <a href="{{ route('cities.restore',$city->id) }}" class="btn btn-warning"><i class="fa fa-arrow-circle-o-left"></i></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
...
@section('script')
    ...
    <script>
        ...
        if(document.getElementById('cities')){
            document.getElementById('cities').classList.add('selected');
        }
    </script>
    <script>
