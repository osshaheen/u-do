...
@section('content')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h4 class="text-themecolor">{{ trans('Places/branches/update.Place_Branch_title') }}</h4>
        </div>
        <div class="col-md-7 align-self-center text-right">
            <div class="d-flex justify-content-end align-items-center">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active"><a href="{{ route('placesBranch.edit',$branch->id) }}">{{ trans('Places/branches/update.Update_Place_Branch') }}</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('places.show',$place->id) }}">{{ trans('Places/branches/update.Place_Branch_title') }}</a></li>
                </ol>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header bg-info">
                    <h4 class="m-b-0 text-white">{{ trans('Places/branches/update.Update_Place_Branch') }}</h4>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('placesBranch.update',$branch->id) }}">
                        @method('put')
                        @csrf
                        @include('control_panel.places.branches._form',['button'=>trans('Places/branches/update.Update_Place_Branch')])
                    </form>
                </div>
            </div>
        </div>
...
@section('script')
    <script>
        ...
        function deleteDetail(detail_id,obj) {
            $.get('/deleteDetail/'+detail_id , function (data, status) {
                // console.log(data);
