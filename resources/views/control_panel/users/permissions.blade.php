...
@section('content')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h4 class="text-themecolor">{{ trans('users/permissions.user_permissions') }}</h4>
        </div>
        <div class="col-md-7 align-self-center text-right">
            <div class="d-flex justify-content-end align-items-center">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active"><a href="{{ route('roles.permissions.user',$user->id) }}">{{ trans('users/permissions.user_permissions') }}</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('users.index') }}">{{ trans('users/permissions.users_title') }}</a></li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">{{ trans('users/permissions.users_title') }}</h4>
                    <div class="table-responsive">
                        <table class="table color-table primary-table">
                            <thead>
                                <tr>
                                    <th>{{ trans('users/permissions.name') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">{{ trans('users/permissions.user_permissions') }}</h4>
                    <div class="table-responsive m-t-40">
                        <table id="myTable" class="table table-bordered table-striped">
                            <thead>
                                <th>
                                    {{ trans('users/permissions.select') }}
                                </th>
                                <th>
                                    {{ trans('users/permissions.name') }}
                                </th>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <input type="checkbox" @if($permission->users_count) checked @endif class="js-switch" data-color="#26c6da" onchange="addPermissionToUser(this,'{{$permission->id}}')"/>
                                    </td>
...
{{--    <script src="{{asset('/control_panel/assets/node_modules/sweetalert/jquery.sweet-alert.custom.js')}}"></script>--}}
    ...
    <script>
        ...
        function addPermissionToUser(obj,permission_id){
            var status = 0;
            obj.checked === true ? status = 1 : status = 0;
            $.get('/addPermissionToUser/'+'{{$user->id}}'+'/'+permission_id+'/'+status , function (data, status) {
                // console.log(data);
            });
        }
